...
import { Component, OnInit, ViewChild, NgZone, ChangeDetectorRef, CUSTOM_ELEMENTS_SCHEMA  } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule, ReactiveFormsModule } from '@angular/forms';
import { IonicModule } from '@ionic/angular';
import { Platform, NavController, AlertController, LoadingController, ToastController, ActionSheetController, IonContent, IonBackButtonDelegate, ModalController } from '@ionic/angular';
...
export class AddServicePage implements OnInit {
 ...
 // @ViewChild(IonSlides) slides: IonSlides;
	...
	public registerServiceForm: FormGroup= new FormGroup({});
	public datos: any;
	public datosU: any;
	public datosE: any;
					...
					for (var j = 0; j < this.items.length; ++j) {
						if (this.categories_principal[i].id == this.items[j].catprincipales_id) {
							for (var m = 0; m < this.items[j].subcategorias.length; ++m) {
								let index = this.items1.findIndex((item2:any) => item2.subcategoria_id === this.items[j].subcategorias[m].id);
								if(index == -1){
	...
	deleteLanguage(item:any){
		let index = this.selecZones.findIndex((item1:any) => item1.id === item.id);
		if(index !== -1){
	...
	copyFileToLocalDir1(fullPath:any, type:any) {
	    let myPath = fullPath;
	...
	public takePicture(sourceType:any, type:any) {
	...
	private copyFileToLocalDir(namePath:any, currentName:any, newFileName:any, type:any) {
	...
	delete_image(item:any){
		let index = this.images_upload.findIndex((itemN:any) => itemN.url === item.url);
		if(index !== -1){
